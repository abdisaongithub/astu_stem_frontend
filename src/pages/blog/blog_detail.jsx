import React from "react";
import {Link} from "react-router-dom";

const BlogDetail = () => {
    return (
        <div className="bg-gray-100">
            <div className="w-full h-[200px] bg-blog_header2 flex text-white">
                <div className="mx-auto my-auto text-8xl">
                    <h1 className="text-white mb-2">Some Blog Title</h1>
                </div>
            </div>
            <div className="mx-32 flex mt-10">
                <div className="flex-col w-1/3 ">
                    <img src="/images/profile.jpg" className="h-[160px] w-[160px] object-cover rounded-full" alt=""/>
                    <p className="font-bold text-2xl my-3">Abdisa Tsegaye</p>
                    <p className="font-bold text-lg text-blue-600 my-3 leading-none">Academic and Research Assistant</p>
                    <p className="font-bold text-lg text-blue-600 my-3 leading-none">ASTU, Adama</p>
                    <hr className="border border-black my-5"/>
                    <p className="font-bold text-2xl my-3">Accomplishments</p>
                    <div className="flex">
                        <div className="h-8 w-8 bg-blue-600 rounded-full mr-5 my-auto">
                            <h1 className="text-white text-center justify-center pt-1 m-0 ">1</h1>
                        </div>
                        <div className="fle-col">
                            <p className="font-bold block text-lg text-blue-600 my-3 leading-none">Academic and Research Assistant</p>
                            <p className="font-bold block text-lg text-blue-600 my-3 leading-none">ASTU, Adama</p>
                        </div>
                    </div>
                </div>
                <div className="flex-col w-2/3 ml-10">
                    <p className="text-semibold text-justify">
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                        Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them. Winds, spirits, and ultimate lotus will always protect them.
                    </p>
                </div>
            </div>
        </div>
    )
}

export default BlogDetail
